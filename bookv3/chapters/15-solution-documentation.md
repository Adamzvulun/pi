...
שרטוט מלא של המערכת מופיע כתרשים נפרד ב־[`bookv3/diagrams/full-schematic.mmd`](../diagrams/full-schematic.mmd). הכותרת המקוצרת: שלוש מסילות כוח (12V מהקיר, 5V לוגיקה מ־USB-C של ה־Pi, 5V סרוו מ־LM2596), אדמה משותפת, ושני מעגלי בקרה (I²C ל־PCA9685 ו־GPIO18 למתג הלייזר).

### חישובי הנגדים

נגד 100Ω בלייזר: לפי הדאטה־שיט של דיודות 5mW 650nm סטנדרטיות, Vf ≈ 3V ו־Id = 20mA נחשבים בטוחים. עם Vsupply = 5V:

`[נוסחה 02 — נגד הגבלת זרם ללייזר]`

נגד 220Ω על Gate של MOSFET: מגן על ה־GPIO של ה־Pi מפני זרמי הסתערות בעת מעבר state. עם VGPIO = 3.3V ו־Imax ≈ 15mA:

`[נוסחה 03 — נגד Gate של MOSFET]`

נגד 100kΩ Pulldown: מספיק נמוך כדי לסגור את ה־MOSFET תוך ~100µs, מספיק גבוה כדי לא לבזבז זרם משמעותי (33µA במצב ON, פחות מ־1mW).
...
| 4 | 5V | 100Ω → Laser(+) | אספקה ללייזר |
| 5 | GPIO3 (SCL) | PCA9685 SCL | I²C clock |
| 6 | GND | PCA9685 GND + Laser GND | אדמה משותפת |
| 12 | GPIO18 | 220Ω → MOSFET Gate | בקרת לייזר |
