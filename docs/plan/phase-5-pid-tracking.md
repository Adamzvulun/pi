...
**Target moves fast, leaves FOV, bracket freezes** — that's the symptom coast mode fixes. `tracker.update()` remembers the last PID correction; when `target_pos` becomes `None` it keeps applying that correction (with a small per-frame decay) for up to `COAST_MAX_FRAMES`. The overlay shows `COASTING (N frames left)` in orange while this is happening. If the bracket consistently can't catch up even with coast, raise `COAST_MAX_FRAMES` (longer coast window) or lower `COAST_DECAY` toward 1.0 (less per-frame slowdown). If the bracket coasts off in odd directions when targets are stationary and then disappear, raise `COAST_MIN_CORRECTION_DEG` so trivial-motion losses don't trigger coast.

**After coast fails the bracket is parked in a corner staring at the wall** — happens when the target moves diagonally fast enough that coast runs the bracket into both pan and tilt limits before re-acquisition. Without intervention the bracket would sit there. Fixed by recenter-after-coast: when coast expires (or both axes are clamped simultaneously during coast), `tracker.update()` flips into recenter mode and ramps the bracket back to `PAN_CENTER` / `TILT_CENTER` a step at a time (`RECENTER_STEP_DEG` degrees per frame, default 2°). The overlay shows `RECENTERING to home` in purple. If the target reappears mid-recenter, PID resumes immediately. Disable by setting `RECENTER_AFTER_COAST = False` in `config.py`. Adjust `RECENTER_STEP_DEG` for slower (gentler-looking) or faster recenters.
