...
def init() -> LED:
    ...
    Hand-offs between processes (e.g. control_panel releasing the pin
    before launching calibrate_boresight): the caller is responsible
    for ensuring the GPIO is fully released first — typically via
    laser.cleanup() AND closing gpiozero's pin_factory entirely so the
    chip handle's kernel-level claim is gone. See control_panel._on_*
